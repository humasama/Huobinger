...
jiffies()
{
	ip_context_t *ip_thread_local_p = pthread_getspecific(ip_context);
...
//!     ip_free(this_host->ipqueue);
		ip_free(ip_thread_local_p->this_host->ipqueue);
	}
}
...
//!   while (timer_head && timer_head->expires < jiffies()) {
	while (ip_thread_local_p->timer_head && ip_thread_local_p->timer_head->expires < jiffies()) {
